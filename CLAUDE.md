...
- Yahoo drops single days per series (e.g. 09-22 for XLK and SPX but not XLE). Never count rows per series to build a window: read the closes on the window's exact first and last dates and show n/a when one is missing. `sector_block` does this over US sessions, the page's `relSeries` counts the stock's own sessions and looks up the ETF and S&P on those dates, and the 2s10s uses the last day both legs have (`spread_on_shared_dates`).
